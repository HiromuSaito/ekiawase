...
export function findNearestStationsToMidpoint(
  ...
  const inputStations = allStations.filter((st) =>
    inputCodes.includes(st.code),
  );
  ...
  // 緯度・経度の合計を求める
  const sum = inputStations.reduce<{ lat: number; lon: number }>(
    (acc, st) => {
      acc.lat += st.lat;
      acc.lon += st.lon;
  ...
  const stationsWithDistance = allStations.map((st) => ({
    ...st,
    distance: haversineDistance(midpointLat, midpointLon, st.lat, st.lon),
  }));
